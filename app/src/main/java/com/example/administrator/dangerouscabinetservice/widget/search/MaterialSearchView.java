...
public class MaterialSearchView extends FrameLayout implements Filter.FilterListener {
    ...
    private void initSearchView() {
        ...
        mSearchSrcTextView.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                mUserQuery = s;
                startFilter(s);//把输入的字符串传递给过滤器
                MaterialSearchView.this.onTextChanged(s);//输入字符串后的相应操作
            }
    ...
    /**
     * Call this method to show suggestions list. This shows up when adapter is set. Call {@link #setAdapter(android.widget.ListAdapter)} before calling this.
     */
